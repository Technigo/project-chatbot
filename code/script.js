...
const greetUser = () => {
  showMessage("Hello dear, my name is Boredom Buddy Bot, what's your name?", 'bot');
}
...
const saveUsername = (event) => {
  ...
  setTimeout(() => showMessage(`${username} is an amazing name!`, 'bot'), 1000);
  setTimeout(() => askPersonality(), 2000); 
}

// Ask the second question now
const askPersonality = () => {
  showMessage(`How would you describe yourself, ${username}?`, 'bot');
  setTimeout(() => personalityTraits(), 1000);
}
...
const personalityChoice = (chosenPersonality) => {
  ...
  showMessage('Awesome, If I’m honest, me too sometimes, but', 'bot');  
  inputWrapper.innerHTML = ''; // Clear the buttons after the user makes a choice
  setTimeout(() => askMorning(), 1000);  // Ensure the next question is asked after a brief delay
}

// Ask the third question now 
const askMorning = () => {
  showMessage(`How do you usually prefer to start your mornings?`, 'bot');
  setTimeout(() => morningPreferences(), 1000); // Give time before showing the preferences
}
...
const morningPreference = (preference) => {
  ...
  setTimeout(() => showMessage(`Interesting! I’m not much of a ${preference} type of bot myself, but okay`, 'bot'), 1000); 
  inputWrapper.innerHTML = ''; // Clear the buttons after the user makes a choice
  setTimeout(() => askStress(), 1000);  // Ensure the next question is asked after a brief delay
}

// Ask the fourth question now 
const askStress = () => {
  showMessage(`If I may ask, how do you usually handle stress?`, 'bot');
...
// Handle the user's stress management and display it
const stressHandle = (handle) => {
  showMessage(handle, 'user');  
  setTimeout(() => showMessage(`That's cute from you to share! I think I prefer ${handle} too.`, 'bot'), 1000); 
  inputWrapper.innerHTML = ''; // Clear the buttons after the user makes a choice
...
const askDream = () => {
  showMessage(`Now for the final question, if you could be any supernatural creature in the world, who would it be?`, 'bot');
...
const dreamSuper = (dream) => {
  ...
  showMessage(`Wow, me too!`, 'bot');  
  setTimeout(() => feedbackRequest(), 1000);
}

// Ask for feedback
const feedbackRequest = () => {
  showMessage(`Thank you ${username} for opening up to me, it means a lot! How did our chat make you feel?`, 'bot');
  inputWrapper.innerHTML = `
...
// Handle feedback based on the user's response
const sendFeedback = (feedback) => {
  if (feedback === 'lovely') {
    showMessage('Thank you lovely ❤️', 'user');
    showMessage('❤️ Sending you all the love! Have a wonderful day! ❤️', 'bot');
  } else if (feedback === 'boring') {
    showMessage('It was a boring chatbot 💔', 'user');
    showMessage('💔 I’m sorry you feel that way. I’ll try to be more fun next time! 💔', 'bot');
  }
