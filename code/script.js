...
const reservation = {
  ...
  movies: ["Barbie (2023)", "Oppenheimer", "Teenage Mutant Ninja Turtles: Mutant Mayhem", "The Little Mermaid (2023)", "Aja baja Alfons Åberg", "Meg 2: The Trench"],
  showtimesAvailable: ["12:00 PM", "3:00 PM", "6:00 PM"],
};

const ticketPrices = {  // in us$
  adultPrice: [11],
  studentPrice: [8],
  childPrice: [4],
};
...
const greetUser = () => {
  ...
  setTimeout(() => {
    showMessage("To proceed, I need the name of the person who makes the reservation. Please, enter your name.", 'bot');
  }, 500);
...
const ticketType = () => {
  ...
  showMessage(`Nice to meet you ${customerName}! Is the movie ticket for a adult, a student or a child (Under 7)?`, 'bot');
  // Two buttons for age choices.
  inputWrapper.innerHTML += `
    <button id="adult" type="submit">Adult</button>
    <button id="student" type="submit">Student</button>
    <button id="child" type="submit">Child</button>
  `;
...
const showtime = () => {
  removeChildrenFunction();
  showMessage(`Good choice! Select one of the available showtimes for ${movieOption}.`, 'bot');
  // Three new buttons
  inputWrapper.innerHTML += `
    <button id="showtimeOne" type="submit">12:00 PM</button>
    <button id="showtimeTwo" type="submit">3:00 PM</button>
    <button id="showtimeThree" type="submit">6:00 PM</button>`;
...
const confirmReservation = () => {
  removeChildrenFunction();
  showMessage(`Nice ${customerName}! You want to make a reservation for ${movieOption} at ${showtimeOption} for a ${ticketOption.toLowerCase()}. It will cost US$ ${price}.`, "bot");
  setTimeout(() => {
