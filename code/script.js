...
const startChatting = () => {
  setTimeout(() => {
    showMessage(`Awesome! Please select the LEGO® theme you're interested in below.`, 'bot')
    selectTheme()
  }, 1000)
}
...
const handleNotReady = message => {
  setTimeout(() => {
    showMessage(`Hello again, are you ready to place your order now?`, 'bot')
  }, 2000)
  document.getElementById('notReady').addEventListener('click', () => {})
}
...
const evaluateThemeSelection = selectedTheme => {
  if (selectedTheme === 'Star Wars™') {
    ...
    `
      ...
      document
        .getElementById('notInterestedBtn')
        .addEventListener('click', () => handleNotInterested())
    }, 1000)
  }
}
...
const starWarsItems = () => {
  setTimeout(() => {
    showMessage(
      `May the force be with you, you made the only valid choice 🥳 ! Choose one of our top picks:`,
      'bot'
    )
    inputWrapper.innerHTML = `
  ...
  <button id="btn3">Death Star</button>
    ...
    document
      .getElementById('btn1')
      .addEventListener('click', () => confirmOrder('Millenium Falcon'))
    document.getElementById('btn2').addEventListener('click', () => confirmOrder('R2D2'))
    document.getElementById('btn3').addEventListener('click', () => confirmOrder('Death Star'))
  }, 1000)
}
...
const confirmOrder = item => {
  ...
  setTimeout(() => {
    showMessage(
      `Excellent choice! One ${item} have now been added to your cart. Are you happy with your order?`,
      'bot'
    )
    inputWrapper.innerHTML = `
